...
def _build_neuropathic_pain_subgraph_dag() -> Tuple[np.ndarray, List[str], int]:
    ...
    cpdag_path = Path(__file__).parent.parent.parent / 'data' / 'benchmarks' / 'neuropathic_pain' / 'cpdag_GT.csv'
...
DATASET_CONFIGS = {
    ...
    'child': {
        ...
        'feature_names': [
            'Disease', 'Age', 'Sick', 'LVH', 'DuctFlow', 'CardiacMixing',
            'LungParench', 'LungFlow', 'Grunting', 'HypDistrib', 'HypoxiaInO2',
            'CO2', 'ChestXray', 'LowerBodyO2', 'RUQO2', 'CO2Report',
            'XrayReport', 'GruntingReport', 'LVHreport',
        ],
        'target_name': 'BirthAsphyxia',
        'target_idx': 0,  # Node 0 in the 20-node DAG
        'true_mb': [0],  # Disease is the only child of BirthAsphyxia (index 0 in features = Disease)
        'has_true_dag': True,
    ...
    'alarm': {
        ...
        'feature_names': [
            ...
            'HRSAT', 'INSUFFANESTH', 'ANAPHYLAXIS', 'TPR', 'EXPCO2',
            'KINKEDTUBE', 'MINVOL', 'FIO2', 'PVSAT', 'SAO2', 'PAP',
            'PULMEMBOLUS', 'SHUNT', 'INTUBATION', 'PRESS', 'DISCONNECT',
            'MINVOLSET', 'VENTMACH', 'VENTTUBE', 'VENTLUNG', 'VENTALV',
...
def _load_sachs() -> Tuple[np.ndarray, np.ndarray, Dict]:
    ...
    if csv_candidates:
        df = pd.read_csv(csv_candidates[0]) if str(csv_candidates[0]).endswith('.csv') else pd.read_excel(csv_candidates[0])
...
def _sample_asia_bn(n_samples: int = 5000, seed: int = 42) -> np.ndarray:
    ...
    p_dysp = np.where(
        bronc & either, 0.9,
        np.where(~bronc & either, 0.7,
                 np.where(bronc & ~either, 0.8, 0.1)))
...
def _load_diabetes() -> Tuple[np.ndarray, np.ndarray, Dict]:
    ...
    if df is None:
        ...
        try:
            ...
            warnings.warn("Loading Pima diabetes from local CSV failed. "
                          "Using a placeholder. Download from Kaggle.")
            raise FileNotFoundError("Pima diabetes not in sklearn")
        except Exception:
            raise FileNotFoundError(
                f"Diabetes data not found. Download from Kaggle: "
                f"https://www.kaggle.com/datasets/uciml/pima-indians-diabetes-database"
            )
...
def _load_heart_disease() -> Tuple[np.ndarray, np.ndarray, Dict]:
    ...
    if npy_dir.exists() and (npy_dir / 'X_train.npy').exists():
        ...
        raise FileNotFoundError(
            f"Heart disease data not found at {npy_dir}. "
...
def load_dataset(name: str) -> Tuple[np.ndarray, np.ndarray, Dict]:
    ...
    if dag is not None and dag.shape[0] > n_vars:
        ...
        if target_idx is not None:
            config['true_dag'] = np.delete(
                np.delete(dag, target_idx, axis=0), target_idx, axis=1
            )
        else:
    ...
    if len(config['feature_names']) > n_vars:
        ...
        if target_idx is not None:
            config['feature_names'] = [
                n for i, n in enumerate(config['feature_names'])
                if i != target_idx
            ]
        else:
