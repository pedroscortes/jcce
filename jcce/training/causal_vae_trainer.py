"""
CausalVAE training utilities.

Provides training step, training loop, and utilities for CausalVAE.
"""
...
class CausalVAETrainState(train_state.TrainState):
    """
    Extended train state for CausalVAE training.

    Attributes:
        key: JAX random key for sampling
        A_true: Ground-truth adjacency matrix (fixed during training)
    """
...
def create_causal_vae_train_state(
    model: CausalVAE,
    A_true: jnp.ndarray,
    learning_rate: float,
    key: jax.random.PRNGKey,
    ...
    """
    Create initial training state for CausalVAE.

    Args:
        model: CausalVAE model
    ...
    Example:
        ...
        >>> A_true = jnp.zeros((10, 10))
        >>> key = jax.random.PRNGKey(0)
        >>> state = create_causal_vae_train_state(
        ...     model, A_true, lr=1e-3, key=key, input_shape=(32, 20)
        ... )
    """
    ...
    variables = model.init(init_key, dummy_input, A_true, dropout_key, training=True)

    # Create optimizer
    tx = optax.adam(learning_rate)

    # Create train state
    state = CausalVAETrainState.create(
...
@jax.jit
def causal_vae_train_step(
    state: CausalVAETrainState, batch: jnp.ndarray, beta: float = 1.0
) -> tuple[CausalVAETrainState, dict]:
    """
    Single training step for CausalVAE.

    Args:
        state: Current training state (contains A_true)
    ...
    Example:
        ...
        >>> new_state, metrics = causal_vae_train_step(state, batch)
        >>> print(metrics.keys())
    ...
    def loss_fn(params):
        # Forward pass with causal layer
        x_recon, info = state.apply_fn(
            {"params": params}, batch, state.A_true, subkey, training=True
        )

        # Compute ELBO loss for CausalVAE
        loss, metrics = causal_vae_elbo_loss(
            x=batch,
...
def causal_vae_train_epoch(
    state: CausalVAETrainState,
    train_data: jnp.ndarray,
    batch_size: int,
    beta: float = 1.0,
) -> tuple[CausalVAETrainState, dict]:
    """
    Train CausalVAE for one epoch.

    Args:
        state: Training state (contains A_true)
    ...
    Example:
        ...
        >>> state, metrics = causal_vae_train_epoch(state, train_data, batch_size=32)
    """
    ...
    for i in range(n_batches):
        ...
        state, metrics = causal_vae_train_step(state, batch, beta=beta)
...
def train_causal_vae(
    ...
    """
    Train CausalVAE for multiple epochs.
    ...
    Example:
        ...
        ...     model, A_true, train_data, n_epochs=10
        ... )
    """
    ...
    for epoch in range(n_epochs):
        ...
        # Validation (optional)
        if val_data is not None:
            val_metrics = evaluate_causal_vae(state, val_data, batch_size, beta=beta)
...
def evaluate_causal_vae_batch(
    ...
    """
    Evaluate CausalVAE on a single batch.
    ...
    Returns:
        metrics: Dictionary with loss components
    """
    # Forward pass (eval mode, use mean)
    x_recon, info = state.apply_fn(
        {"params": state.params}, batch, state.A_true, key, training=False
    )
...
def evaluate_causal_vae(
    state: CausalVAETrainState, data: jnp.ndarray, batch_size: int, beta: float = 1.0
) -> dict:
    """
    Evaluate CausalVAE on dataset.
    ...
    for i in range(n_batches):
        ...
        metrics = evaluate_causal_vae_batch(state, batch, subkey, beta=beta)
