...
class UnifiedCVResult:
    ...
    def summary(self) -> str:
        ...
        if self.edge_f1_per_fold is not None:
            ...
            lines.append(f"  Edge F1:      {ef1_mean:.4f} ± {ef1_std:.4f} "
                         f"(P={ep_mean:.4f}, R={er_mean:.4f})")
        if self.total_time is not None:
...
def _compute_shd(A_est: np.ndarray, A_true: np.ndarray, threshold: float = EDGE_THRESHOLD_DEFAULT) -> int:
...
def _evaluate_single_fold(
    ...
    with ctx:
        ...
        result = {
            ...
            'mb_indices': [], 'A_est': None,
            'shd': None, 'mb_f1': None,
            'fold_time': 0.0, 'success': False,
        }

        try:
            # Always use v7 (v4_joint has a pos_embed shape bug with
            # Transformer when augmented data includes Y).
            A_est, processor_trained, proc_params, fold_metrics = \
                learn_structure(
                    data=X_train_jax,
                    Y=Y_train_jax,
                    Y_idx=Y_idx,
        ...
        try:
            ...
            if processor_trained.__class__.__name__ == 'GNNAdapter':
                ...
                Y_pred_logits = processor_trained.forward(
                    X_test_weighted, proc_params_Y, A=A_norm
                )
            else:
                Y_pred_logits = processor_trained.forward(
                    X_test_weighted, proc_params_Y
                )
        ...
        if verbose:
            if task == 'regression':
                print(f"    R²={result['accuracy']:.4f}, RMSE={result['f1']:.4f}, "
...
def evaluate_pareto_solution_cv(
    ...
    if use_parallel and verbose:
        ...
        print(f"  Parallel CV requested but <2 GPUs detected, falling back to sequential")

    # Common kwargs for _evaluate_single_fold
    common_kwargs = dict(
        X=X, Y=Y, hyperparams=hyperparams, processor_type=processor_type,
        A_init=A_init, n_features=n_features, Y_idx=Y_idx, n_folds=n_folds,
        golem_max_iter=golem_max_iter, cold_start=cold_start, task=task,
        use_v7=use_v7, true_dag=true_dag, true_mb=true_mb, verbose=verbose,
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for fold_idx, (train_idx, test_idx) in enumerate(fold_splits):
                ...
                future = executor.submit(
                    ...
                    fold_key=fold_keys[fold_idx], proc_key=proc_keys[fold_idx],
                    device=device, **common_kwargs,
                )
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    fold_results[fidx] = {
                        'fold_idx': fidx, 'accuracy': 0.0, 'f1': 0.0,
                        'balanced_acc': 0.5 if task == 'classification' else 0.0,
                        'roc_auc': 0.5 if task == 'classification' else 0.0,
                        'mb_indices': [], 'A_est': None,
                        'shd': None, 'mb_f1': None,
