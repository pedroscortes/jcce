...
from config import DATASETS, PROCESSOR_CONFIGS, CV_CONFIG, SHAP_CONFIG, BASE_DATA_PATH, RESULTS_PATH
...
def run_baseline_for_dataset(
        ...
        for proc_name in processors:
            ...
            try:
                metrics = run_classification_cv(
                    X_subset, y, proc_name, proc_config,
    ...
    if jcce_mb is not None:
        ...
        if verbose:
            ...
            if config.true_mb:
                print(f"\n  vs Ground Truth:")
                print(f"    SHAP: P={comparison['ground_truth']['shap']['precision']:.3f}, "
                      f"R={comparison['ground_truth']['shap']['recall']:.3f}, "
                      f"F1={comparison['ground_truth']['shap']['f1']:.3f}")
                print(f"    MB:   P={comparison['ground_truth']['mb']['precision']:.3f}, "
    ...
    for feature_set in results['classification']:
        ...
        for proc, metrics in results['classification'][feature_set].items():
            if 'error' in metrics:
                ...
                auc = f"{metrics.get('roc_auc_mean', np.nan):.3f}" if not np.isnan(metrics.get('roc_auc_mean', np.nan)) else "N/A"
    ...
    if results['comparison']:
        ...
        if 'shap_vs_jcce' in results['comparison']:
            ...
            if 'ground_truth' in comp:
                ...
                report.append(f"| SHAP | {shap_gt['precision']:.3f} | {shap_gt['recall']:.3f} | {shap_gt['f1']:.3f} |")
                report.append(f"| JCCE MB | {mb_gt['precision']:.3f} | {mb_gt['recall']:.3f} | {mb_gt['f1']:.3f} |")
...
def main():
    ...
    parser.add_argument('--jcce-results', type=str, default=None, help='Path to JCCE results pickle')
