...
def _find_base_path() -> Path:
    ...
    raise RuntimeError(
        "Could not locate jcce repo root (no pyproject.toml found within 10 parent levels)."
    )
...
def run_baselines(dataset_name: str, device="cuda"):
    ...
    # Run classifiers
    header = (
        f"{'Model':<15} {'Features':<12} {'Accuracy':<15} {'Precision':<15} "
    ...
    for model_type in ["mlp", "transformer", "mamba", "gnn", "elm"]:
        for feat_set, feat_name in [("all", "All"), ("shap", f"SHAP-{k}")]:
            ...
            print(
                f"{model_type.upper():<15} {feat_name:<12} {metrics['accuracy']:<15} "
...
def main():
    ...
    parser.add_argument(
        "--all", action="store_true", help=f"Run target datasets: {DEFAULT_DATASETS}"
    )
    parser.add_argument("--list", action="store_true", help="List available datasets")
    parser.add_argument("--device", type=str, default="cuda")
    parser.add_argument(
        "--base-path", type=str, default=None, help="Override auto-detected project base path"
    )
