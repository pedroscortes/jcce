...
def main():
    parser = argparse.ArgumentParser(
        description='Compare NSGA-II vs Optuna for JCCE'
    )
    parser.add_argument('--dataset', type=str, default='synthetic',
                        choices=['lucas', 'heart_disease', 'breast_cancer',
                                 'diabetes', 'sachs', 'asia', 'synthetic'])
    parser.add_argument('--n-evals', type=int, default=10,
                        help='Total evaluations per method')
    parser.add_argument('--max-iter', type=int, default=30,
                        help='Max GOLEM iterations per evaluation')
    parser.add_argument('--seed', type=int, default=42)
    parser.add_argument('--quiet', action='store_true')
    parser.add_argument('--output', type=str, default=None,
                        help='Save results JSON to file')
    parser.add_argument('--nsga2-results', type=str, default=None,
                        help='Load cached NSGA-II results from JSON file '
                             '(skip re-running NSGA-II)')
    ...
    print(f"Comparing NSGA-II vs Optuna")
    ...
    if args.nsga2_results:
        ...
        nsga2_result = run_nsga2(X, Y, n_vars, args.n_evals, args.max_iter,
                                 args.seed, verbose)
    optuna_result = run_optuna(X, Y, n_vars, args.n_evals, args.max_iter,
                               args.seed, verbose)
    ...
    for res in [nsga2_result, optuna_result]:
        ...
        if sols:
            obj_points = np.array([
                [s['metrics']['classification_balanced_accuracy'],
                 s['metrics']['mb_sparsity']]
                for s in sols
            ])
        else:
            ...
            for s in sols:
                ...
                if A_est is not None:
                    ...
                    sm = compute_structure_metrics(
                        jnp.array(A_est_sub), jnp.array(A_true_sub))
    ...
    print(f"{'='*70}")
    for metric in ['n_pareto', 'n_evals', 'wall_time', 'hypervolume',
                    'best_bacc', 'mean_bacc', 'mean_sparsity', 'n_stable_edges',
                    'best_f1', 'best_shd', 'best_sid']:
