...
def main():
    ...
    print(f"ATE QUALITY EVALUATION (C.10)")
    print(f"=" * 70)
    ...
    print(f"Ground truth ATEs (Xi->Y):")
    ...
    # Summary
    print(f"\n{'=' * 70}")
    print(f"ATE QUALITY SUMMARY\n")

    valid_results = [r for r in ate_results if r.get('n_matched', 0) > 0]

    if valid_results:
        print(f"{'Sol#':>5} {'BAcc':>6} {'PEHE':>8} {'MeanErr':>8} "
              f"{'ATEBias':>8} {'#Match':>7}")
        print(f"{'-' * 5} {'-' * 6} {'-' * 8} {'-' * 8} {'-' * 8} {'-' * 7}")
        ...
        for i, (sol, ate_r) in enumerate(zip(enhanced_solutions, ate_results)):
            if ate_r.get('n_matched', 0) > 0:
                ...
                print(f"{i:>5} {bacc:>6.3f} {ate_r['pehe']:>8.4f} "
        ...
        for v in sorted(true_ates.keys()):
            if v < args.n_vars and v in best_r.get('pred_ates', {}):
                ...
                true = true_ates[v]
                err = abs(pred - true)
                print(f"  X{v}->Y: true={true:+.4f} pred={pred:+.4f} "
                      f"err={err:.4f}")
    else:
        ...
        output = {
            ...
            'ate_results': [
                {k: v for k, v in r.items()
                 if k not in ('ate_errors', 'pred_ates')}
