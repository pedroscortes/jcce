...
def run_ablation_condition(cond, X, Y, n_vars, A_true, n_trials, max_iter, seed):
    ...
    golem_overrides = {
        k: v for k, v in cond.get("golem_overrides", {}).items() if k != "lambda_class_override"
    }
    ...
    enhanced_solutions, _ = extract_pareto_solutions(study, use_v7=cond["use_v7"])
    ...
    print(
        f"    HV={hv:.3f} BAcc={best_bacc:.3f}(best)/{mean_bacc:.3f}(mean) "
...
def main():
    ...
    print(
        f"{'Condition':<22} {'HV':>6} {'BestBAcc':>9} {'MeanBAcc':>9} "
    ...
    for r in all_results:
        print(
            f"{r['name']:<22} {r['hv']:>6.3f} {r['best_bacc']:>9.3f} "
    ...
    for r in all_results[1:]:
        ...
        print(
            f"  {r['name']:<22} deltaHV={delta_hv:+.3f} "
