...
def run_noise_condition(X, Y, n_vars, A_true, n_trials, max_iter, seed, noise_scale, noise_type):
    ...
    if enhanced_solutions:
        ...
        stability_result = {
            ...
            "f1": 0.0,
            "n_stable": 0,
            "n_true": int(np.sum(np.abs(A_true) > 1e-6)),
            "n_stable_08": 0,
        }
    ...
    if enhanced_solutions:
        for sol in enhanced_solutions:
            ...
            if A_est is not None:
                ...
                sm = compute_structure_metrics(jnp.array(A_est_X), jnp.array(A_true))
                if sm["shd"] < best_shd:
                    best_shd = sm["shd"]
                    best_f1 = sm["f1"]
...
def main():
    ...
    parser.add_argument("--noise-scales", nargs="+", type=float, default=[0.1, 0.3, 0.5, 1.0, 2.0])
    parser.add_argument("--noise-types", nargs="+", default=["gaussian", "laplace"])
    ...
    print("NOISE SENSITIVITY ANALYSIS (C.7)")
    print("=" * 70)
    ...
    for noise_type in args.noise_types:
        ...
        for noise_scale in args.noise_scales:
            ...
            print(
                f" HV={result['hv']:.3f} bestSHD={result['best_shd']} "
    ...
    print(f"\n{'=' * 70}")
    print(
        f"SUMMARY: {args.n_vars} vars, {n_edges} true edges, "
        f"{args.n_trials} trials x {args.max_iter} iters\n"
    )
    print(
        f"{'Noise Type':<10} {'Scale':>6} {'HV':>6} {'BestSHD':>8} "
    ...
    for r in all_results:
        print(
            f"{r['noise_type']:<10} {r['noise_scale']:>6.1f} "
    ...
    for noise_type in args.noise_types:
        ...
        if len(type_results) >= 2:
            ...
            print(
                f"  {noise_type}: HV drops {hv_drop:.1f}% from "
                f"noise={scales[0]} to noise={scales[-1]}"
            )
            print(
                f"  {noise_type}: F1 drops {f1_drop:.1f}% from "
                f"noise={scales[0]} to noise={scales[-1]}"
            )
