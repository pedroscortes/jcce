...
def main():
    ...
    print("DATASET SHIFT ROBUSTNESS (C.8)")
    print("=" * 70)
    ...
    for cond in shift_conditions:
        ...
        print(
            f"  Train: {n_pareto} Pareto solutions, "
        ...
        if enhanced_solutions:
            train_stab = compute_edge_stability(enhanced_solutions, args.n_vars, threshold=0.1)
            train_gt = compare_stability_to_ground_truth(train_stab, A_true_full, min_frequency=0.5)
        else:
        ...
        if enhanced_solutions:
            ...
            print(
                f"  Test:  {test_feasible}/{n_pareto} feasible, "
    ...
    print("SUMMARY\n")
    print(
        f"{'Condition':<20} {'TrainBAcc':>10} {'TestBAcc':>9} {'Drop':>7} "
        f"{'TrainStab':>10} {'TestStab':>9} {'Drop':>7}"
    )
    print(f"{'-' * 20} {'-' * 10} {'-' * 9} {'-' * 7} {'-' * 10} {'-' * 9} {'-' * 7}")

    for r in all_results:
        print(
            f"{r['condition']:<20} {r['train_bacc']:>10.3f} "
    ...
    for r in all_results:
        ...
        if control and r["bacc_drop"] > control["bacc_drop"] + 0.05:
            print(
                f"  {r['condition']}: BAcc degrades significantly "
                f"(drop={r['bacc_drop']:+.3f} vs control={control['bacc_drop']:+.3f})"
            )
        else:
            print(f"  {r['condition']}: BAcc robust (drop={r['bacc_drop']:+.3f})")
