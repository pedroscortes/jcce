...
from jcce.utils.metrics import compute_sid, compute_structure_metrics
...
def run_condition(
    name, X, Y, n_vars, n_trials, max_iter, seed, use_pruning, use_conditional, A_true=None
):
    ...
    if A_true is not None and enhanced_solutions:
        ...
        for sol in enhanced_solutions:
            ...
            if A_est is not None:
                ...
                sm = compute_structure_metrics(jnp.array(A_est), jnp.array(A_true_sub))
...
def main():
    ...
    print("MULTI-FIDELITY ABLATION (C.9)")
    print("=" * 70)
    ...
    print(f"\n{'=' * 70}")
    print(
        f"{'Condition':<25} {'HV':>6} {'Time(s)':>8} {'Feasible':>9} "
    ...
    for r in condition_results:
        ...
        print(
            f"{r['name']:<25} {r['hv']:>6.3f} {r['time']:>8.1f} "
    ...
    delta_time_prune = (
        (time_prune - time_no_prune) / time_no_prune * 100 if time_no_prune > 0 else 0
    )
    ...
    hv_flat = np.mean(
        [
            r["hv"]
            for r in condition_results
            if "flat" in r["name"] and "conditional" not in r["name"]
        ]
    )
