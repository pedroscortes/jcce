...
from jcce.data.scm import LinearSCM, NonlinearMLPSCM, SCMConfig
...
def generate_scaling_data(
    ...
    for attempt in range(max_attempts):
        ...
        if n_parents_y >= min_y_parents:
            if attempt > 0:
                print(
                    f" [DGP rejected {attempt} seed(s), using seed={current_seed}]",
                    end="",
                    flush=True,
                )
            break
    ...
    return (jnp.array(X, dtype=jnp.float32), Y_binary, A_true, n_edges, vs)
...
def run_single_config(X, Y, n_vars, A_true, n_trials, max_iter, seed):
    ...
    n_feasible = len(
        [
            ...
            if t.state == optuna.trial.TrialState.COMPLETE and t.user_attrs.get("h_A", 1.0) < 0.1
        ]
    )
...
def main():
    ...
    parser.add_argument("--graph-types", type=str, nargs="+", default=["erdos_renyi", "scale_free"])
    parser.add_argument("--degrees", type=float, nargs="+", default=[2.0, 4.0])
    parser.add_argument("--scm-types", type=str, nargs="+", default=["linear", "nonlinear"])
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Quick mode: d={5,10}, ER-2 only, 5 trials, 20 iters"
    )
    ...
    for cond in conditions:
        ...
        for d in args.dims:
            ...
            print(
                f" | HV={result['hv']:.3f} BAcc={result['best_bacc']:.3f} "
    ...
    for cond in conditions:
        ...
        print(f"\n{cond['label']}:")
        print(
            f"  {'d':>5} {'Edges':>6} {'HV':>7} {'BAcc':>6} {'SHD':>5} "
    ...
    for cond in conditions:
        cond_results = sorted(
            [r for r in all_results if r["condition"] == cond["label"]], key=lambda x: x["d"]
        )
        if len(cond_results) >= 2:
            ...
            print(
                f"  {cond['label']}: time scales as d^{alpha:.2f} "
