...
def run_scaling_experiment(dimensions: list, n_per_family: int = 30, seed: int = 42):
    ...
    print(f"\n{'='*80}")
    print(f"  GBS Kernel Scaling Crossover Analysis")
    ...
    for d in dimensions:
        ...
        # Classical methods
        for name, metric_fn in [('SHD', lambda a, b: float(shd(a, b))),
...
def analyze_crossover(all_results: dict):
    ...
    for d in dims:
        ...
        best_classical = max(
            all_results[d][m]['ari']
            for m in ['SHD', 'Frobenius', 'Spectral', 'Jaccard']
        )
        best_classical_name = max(
            ['SHD', 'Frobenius', 'Spectral', 'Jaccard'],
            key=lambda m: all_results[d][m]['ari']
        )
        delta = gbs_ari - best_classical
        winner = 'GBS' if delta > 0 else 'Classical'
        print(f"    d={d:>2}: GBS o3={gbs_ari:.4f} vs {best_classical_name}={best_classical:.4f} "
