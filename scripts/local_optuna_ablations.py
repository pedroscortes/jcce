...
def run_one_config(dataset_name, config_name, golem_overrides, n_trials=20, max_iter=100):
    ...
    try:
        ...
        print(
            f"      BAcc={best_bacc:.3f}, CV={cv_bacc:.3f}±{cv_std:.3f}, "
...
def main():
    ...
    for ds in args.datasets:
        ...
        for config_name, overrides in configs:
            r = run_one_config(
                ds, config_name, overrides, n_trials=args.n_trials, max_iter=args.max_iter
            )
    ...
    print(f"{'=' * 90}")
    print(
        f"{'Config':<30s} {'Dataset':<12s} {'BAcc':>6s} {'CV BAcc':>10s} {'DML sig':>8s} {'Time':>6s}"
    )
    print("-" * 75)
    for r in all_results:
        if "error" in r:
            ...
            continue
        print(
            f"{r['config']:<30s} {r['dataset']:<12s} {r['best_bacc']:>6.3f} "
