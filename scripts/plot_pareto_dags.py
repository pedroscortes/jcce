        ...
        for idx in keep:
            ...
            for other in keep:
                ...
                if A_confound is not None:
                    if abs(A_confound[idx, other]) >= threshold or abs(A_confound[other, idx]) >= threshold:
                        has_edge = True
                        break
            if has_edge:
                new_keep.append(idx)
...
def compute_structure_metrics(A_est, A_true, threshold=0.01):
    ...
    return {'shd': shd, 'edge_f1': f1, 'edge_precision': precision, 'edge_recall': recall,
            'tp': tp, 'fp': fp, 'fn': fn}
...
def get_diverse_solutions(solutions: list, feature_names: list, n_max: int = 5):
    ...
    good_acc = [s for s in sol_data if s['bacc'] >= 0.60 and s['idx'] not in used_indices and s['mb_size'] >= 1]
    if good_acc:
        for candidate in sorted(good_acc, key=lambda x: x['mb_size']):
            ...
            A = A_est * A_w
            A_cb = np.array(sol['A_confound']) if sol.get('A_confound') is not None else np.zeros_like(A)
            A_cw = np.array(sol.get('A_confound_weights')) if sol.get('A_confound_weights') is not None else A_cb
...
def plot_solution(
    ...
    A_confound_weights = np.array(sol.get('A_confound_weights')) if sol.get('A_confound_weights') is not None else None
    ...
    keep_indices = filter_mb_no_isolated(A_direct, A_confound, mb_indices, outcome_idx, VIZ_THRESHOLD)
    ...
    n_direct = sum(1 for i in range(n_keep) for j in range(n_keep)
    ...
    for i in range(n_keep):
        for j in range(i + 1, n_keep):
            if A_confound_filtered is not None and abs(A_confound_filtered[i, j]) >= VIZ_THRESHOLD:
                has_directed = (abs(A_filtered[i, j]) >= VIZ_THRESHOLD or
                                abs(A_filtered[j, i]) >= VIZ_THRESHOLD)
                if not has_directed:
                    n_confound_vis += 1
    ...
    print(f"    Processor={proc} | BAcc={bacc:.4f} | MB={original_mb_size} | Visible={n_visible} | Removed={n_removed}")
    print(f"    Directed edges: {n_direct} | Confounding: {n_confound_vis}")

    # Per-solution post-hoc results (if available)
    if 'cv' in sol:
        cv = sol['cv']
        print(f"    CV: BAcc={cv['balanced_acc_mean']:.4f}+-{cv['balanced_acc_std']:.4f} | "
    ...
    if 'cf' in sol:
        cf = sol['cf']
        print(f"    CF: {cf['n_valid']}/{cf['n_instances']} valid (sparsity={cf['avg_sparsity']:.1f})")
    ...
    if dataset_name == 'lucas':
        ...
        if gt_path.exists():
            ...
            print(f"    SHD={sm['shd']} | Edge F1={sm['edge_f1']:.3f} | Prec={sm['edge_precision']:.3f} | Rec={sm['edge_recall']:.3f}")

        true_mb = ds_config.get('true_mb')
        if true_mb:
            mm = compute_mb_metrics(mb_indices, true_mb)
            print(f"    MB F1={mm['mb_f1']:.3f} | Prec={mm['mb_precision']:.3f} | Rec={mm['mb_recall']:.3f}")
...
def process_dataset(
    ...
    if pareto_sols:
        ...
        if sol_indices is not None:
            ...
            for idx in sol_indices:
                if idx < 0 or idx >= len(pareto_sols):
                    ...
                    continue
                plot_solution(pareto_sols[idx], idx, f"sol{idx}",
                              dataset_name, feature_names, ds_config, output_format,
            ...
            for sol_idx, sol, label in diverse:
                plot_solution(sol, sol_idx, label,
                              dataset_name, feature_names, ds_config, output_format,
                              output_dir=output_dir)
    elif 'model' in result and result['model'] is not None:
        # Fallback: only best solution available (old pkl format)
        print(f"  No pareto_solutions data — plotting best solution only")
        print(f"  (Re-run experiment with updated code to get all Pareto DAGs)")
        model = result['model']
        ...
        plot_solution(model, 0, 'best_accuracy',
                      dataset_name, feature_names, ds_config, output_format,
                      output_dir=output_dir)
    else:
        print(f"  [SKIP] No model artifacts in result")
...
def main():
    ...
    parser.add_argument('--sol', type=int, nargs='+', metavar='IDX',
                        help='Specific solution indices to plot')
