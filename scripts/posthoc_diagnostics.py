...
def compute_residual_normality(
    sol: dict, X: np.ndarray, n_test: int = 5, n_sample: int = 200
) -> dict:
...
def analyze_dataset(
    dataset_name: str, pkl_path: Path, log_path: Path = None, X: np.ndarray = None
) -> dict:
    ...
    if log_path and log_path.exists():
        ...
        if gd["summary"]:
            ...
            for name, key in [
                ("recon vs class", "cos_recon_class"),
                ("recon vs effect", "cos_recon_effect"),
                ("class vs effect", "cos_class_effect"),
            ]:
                st = s[key]
                print(
                    f"    {name:20s}: mean={st['mean']:+.4f} std={st['std']:.4f} "
    ...
    for i, sol in enumerate(sols[:5]):
        ...
        if "error" in bfv:
            ...
            print(
                f"    Sol {i} ({proc:12s} BAcc={bacc:.3f}): {status} "
...
def main():
    ...
    parser.add_argument(
        "--log-dir", type=str, default=None, help="Log directory (auto-detected if not set)"
    )
    parser.add_argument(
        "--dataset", type=str, default=None, help="Single dataset (default: all available)"
    )
    ...
    for source in sources:
        ...
        if args.dataset:
            ...
            datasets = sorted(
                d.name for d in results_dir.iterdir() if d.is_dir() and (d / pkl_pattern).exists()
            )
    ...
    print(f"{'=' * 80}")
    print(
        f"{'Dataset':18s} {'Grad Entries':>12s} {'cos(r,c)':>10s} {'cos(c,e)':>10s} "
