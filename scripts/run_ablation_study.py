...
from jcce.validation.posthoc_pipeline import PostHocValidationPipeline
...
def run_pipeline_baseline(
    ...
    if A_learned is not None and A_learned.shape[0] >= n_vars:
        ...
        edge_importance = np.sum(np.abs(A_learned[:n_vars, :n_vars]), axis=0) + \
                         np.sum(np.abs(A_learned[:n_vars, :n_vars]), axis=1)
    ...
    if verbose:
        ...
        print(f"      Balanced Acc: {result['balanced_acc_mean']:.4f} +/- {result['balanced_acc_std']:.4f}")
...
from jcce.validation.counterfactual_runner import (
    ...
    convert_params_to_jax as _convert_params_to_jax,
...
def _select_diverse_posthoc(pareto_solutions: list, n_max: int = 5) -> list:
    ...
    best_idx = max(range(len(pareto_solutions)),
                   key=lambda i: pareto_solutions[i].get('balanced_accuracy', 0))
    selected.append(best_idx)
    used.add(best_idx)

    # 2. Sparsest with reasonable accuracy
    candidates = [(i, s) for i, s in enumerate(pareto_solutions)
    ...
    remaining = sorted(
        ...
        key=lambda x: x[1].get('balanced_accuracy', 0), reverse=True,
    )
...
def run_ablation(
    ...
    if verbose:
        print(f"Sample split: {X_struct.shape[0]} structure (NSGA-II) / "
    ...
    for i, m in enumerate(raw_sol_metrics):
        bacc = m.get('classification_balanced_accuracy',
        ...
        for i, m in enumerate(raw_sol_metrics):
            ...
            try:
                ...
                dml_result = run_multi_parent_dml(
                    ...
                    A_est=A_est_dml, Y_idx=Y_idx_dml,
        ...
        for i, m in enumerate(raw_sol_metrics):
            ...
            try:
                ...
                if ae_result is not None:
                    ...
                    pareto_solutions_data[i]['dml_causal_effects'] = ae_result.to_causal_effects_dict()
                    if verbose:
    ...
    for i in diverse_indices:
        ...
        try:
            ...
            if cv_result.mb_f1_per_fold:
                pareto_solutions_data[i]['cv']['mb_f1_mean'] = float(np.mean(cv_result.mb_f1_per_fold))
            if verbose:
                star = " *" if i == best_pareto_idx else ""
                print(f"    Sol {i}: CV BAcc={cv_result.balanced_acc_mean:.4f}"
    ...
    if cv_best_idx >= 0 and cv_best_bacc > 0:
        if verbose and cv_best_idx != best_pareto_idx:
            print(f"    Best by CV BAcc: idx={cv_best_idx} (CV={cv_best_bacc:.4f}) "
    ...
    for i in diverse_indices:
        ...
        try:
            ...
            if cf_result:
                ...
                if verbose:
                    star = " *" if i == best_pareto_idx else ""
                    print(f"    Sol {i}: CF {cf_result['n_valid']}/{cf_result['n_instances']} valid "
    ...
    if best_pareto_idx >= 0 and best_sol_metrics is not None:
        ...
        try:
            ...
            _dml_results = []
            best_ps = pareto_solutions_data[best_pareto_idx] if best_pareto_idx < len(pareto_solutions_data) else {}
            if 'dml' in best_ps:
            ...
            if verbose and structural_validation:
                if 'bootstrap' in structural_validation and 'error' not in structural_validation['bootstrap']:
    ...
    result = {
        ...
        'dataset_config': {k: v for k, v in ds_config.items()
                          if k not in ('true_dag',)},
...
def print_summary_table(all_results: list):
    ...
    # Header
    print(f"{'ID':<4} {'Name':<25} {'Dataset':<12} {'CV BAcc':<16} {'CV AUC':<16} "
    ...
    for r in sorted(all_results, key=lambda x: x['ablation_id']):
        ...
        print(f"{ablation_id:<4} {name:<25} {dataset:<12} {bacc:<16} {auc:<16} "
...
ALL_DATASETS = ['lucas', 'diabetes', 'heart_disease', 'breast_cancer', 'sachs', 'asia', 'child', 'neuropathic_pain', 'alarm', 'insurance']
...
def main():
    ...
    parser.add_argument('--pop-size', type=int, default=None, help='Override population size (e.g., 15)')
                ...
                for sol_idx in sol_indices:
                    ...
                    try:
                        ...
                        if all_edges_result is not None:
                            result['pareto_solutions'][sol_idx]['all_edges_dml'] = all_edges_result.to_storage_dict()
                            result['pareto_solutions'][sol_idx]['dml_causal_effects'] = all_edges_result.to_causal_effects_dict()
        ...
        for dataset_name in datasets:
            ...
            for ablation_id in ablations:
                ...
                for sol_idx in sol_indices:
                    ...
                    print(f"  Processor: {ps['processor_type']}, edges={ps['n_edges']}, "
                    ...
                    try:
                        ...
                        with open(result_path, 'wb') as f:
                            pickle.dump(result, f)
                        print(f"  CV BAcc={cv_result.balanced_acc_mean:.4f}"
        ...
        for dataset_name in datasets:
            for ablation_id in ablations:
                ...
                for sol_idx in sol_indices:
                    ...
                    ckpt_path = RESULTS_DIR / dataset_name / f'{ablation_id}{suffix}_cf_checkpoint{ckpt_suffix}.pkl'

                    if ckpt_path.exists():
                        print(f"  Loading checkpoint: {ckpt_path}")
                        X, Y, A_est, A_weights, A_confound, processor, params, ds_config = \
                            _load_cf_checkpoint(ckpt_path)
                    elif (result and 'pareto_solutions' in result
                        ...
                        processor, params = _reconstruct_processor(
                            ps['processor_type'], ps.get('processor_config', {}),
                            ps['processor_params'],
                        )
                    else:
                    ...
                    if cf_result and result is not None:
                        ...
                        print(f"  CF completed but no result file to update")
