...
ALL_DATASETS = [
    'lucas', 'heart_disease', 'breast_cancer', 'asia', 'diabetes',
...
def dag_to_enhanced_solution(dag_info: dict, n_features: int,
    ...
    from jcce.structure_learning.markov_blanket import extract_markov_blanket
...
def threshold_sweep_dag(A_continuous: np.ndarray, thresholds: list,
                        true_dag: np.ndarray = None) -> list:
...
def run_validation_for_baseline(
    ...
    if run_cv:
        ...
        try:
            cv_results = run_posthoc_cv(
                ...
                X=X_effect, Y=Y_effect.astype(np.float32),
                n_folds=n_cv_folds,
                max_solutions=1,
    ...
    if run_threshold_sweep and method in ['dagma', 'notears', 'golem', 'directlingam', 'castle']:
        ...
        if verbose:
            best = max(result['threshold_sweep'],
                       key=lambda x: x.get('edge_f1', 0))
...
def main():
    ...
    parser.add_argument('--methods', nargs='+', default=None,
                        help=f'Methods to validate (default: all). '
                             f'Options: {STRUCTURE_METHODS + ["castle"]}')
    ...
    print(f"{'='*80}")
    print(f"{'Dataset':<20s} {'Method':<15s} {'CV BAcc':>10s} {'DML sig':>10s} "
          f"{'Edges':>8s} {'MB':>5s}")
    print('-' * 70)
    for ds, ds_results in all_results.items():
        for method, res in ds_results.items():
            if isinstance(res, dict) and 'error' not in res:
                ...
                mb = res.get('mb_size', '?')
                print(f'{ds:<20s} {method:<15s} {cv_bacc:>10s} {dml_str:>10s} '
                      f'{edges:>8} {mb:>5}')
