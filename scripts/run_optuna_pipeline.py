...
def _convert_optuna_to_ablation_format(result, dataset_name, args):
    ...
    if best_sol_metrics:
        ...
        if "_processor_params" in best_sol_metrics:
            ...
            model_dict["processor_params"] = convert_params_to_numpy(
                best_sol_metrics["_processor_params"]
            )
    ...
    if getattr(args, "rerun_cv", False):
        ...
        for sol_idx in sol_indices:
            ...
            try:
                ...
                print(
                    f"    Sol {sol_idx}: CV BAcc={cv_result.balanced_acc_mean:.4f}"
        ...
        for sol_idx in sol_indices:
            ...
            try:
                ...
                cf_result = run_counterfactual_evaluation(
                    ...
                    ds_config=ds_config,
                    A_weights=np.array(ps["A_weights"])
                    if ps.get("A_weights") is not None
                    else None,
                    A_confound=np.array(ps["A_confound_weights"])
                ...
                if cf_result:
                    ...
                    print(
                        f"    Sol {sol_idx}: CF {cf_result['n_valid']}/{cf_result['n_instances']} valid"
                    )
            except Exception as e:
...
def main():
    ...
    parser.add_argument("--max-iter", type=int, default=100, help="Max GOLEM iterations per trial")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument(
        "--task", type=str, default="classification", choices=["classification", "regression"]
    )
    parser.add_argument(
        "--use-pc-warmstart", action="store_true", help="Run PC algorithm for warm-start"
    )
    parser.add_argument("--n-folds", type=int, default=5, help="CV folds")
    parser.add_argument(
        "--cv-max-solutions", type=int, default=0, help="Max Pareto solutions for CV (0=all)"
    )
    parser.add_argument(
        "--dml-max-solutions", type=int, default=0, help="Max Pareto solutions for DML (0=all)"
    )
    parser.add_argument("--no-cv", action="store_true", help="Skip post-hoc CV (on by default)")
    parser.add_argument("--no-dml", action="store_true", help="Skip post-hoc DML (on by default)")
    parser.add_argument(
        "--no-cf",
        action="store_true",
    ...
    parser.add_argument(
        "--no-all-edges-dml", action="store_true", help="Skip all-edges DML (on by default)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--storage", type=str, default=None, help="Optuna storage URL (e.g. sqlite:///study.db)"
    )
    ...
    parser.add_argument(
        "--rerun-dml", action="store_true", help="Re-run only DML from saved result pkl"
    )
    parser.add_argument(
        "--rerun-all-edges-dml",
        action="store_true",
    ...
    parser.add_argument(
        "--rerun-cv", action="store_true", help="Re-run only CV from saved result pkl"
    )
    parser.add_argument(
        "--rerun-cf", action="store_true", help="Re-run only CF from saved result pkl"
    )
    parser.add_argument(
        "--sol",
        type=str,
    ...
    print("JCCE Optuna Optimization")
    print(f"  Dataset: {args.dataset}")
    print(f"  Trials: {args.n_trials}, Max iter: {args.max_iter}")
    print(f"  Seed: {args.seed}")

    # Load data
    X, Y, true_mb, true_graph, n_vars, feature_names = load_dataset(args.dataset, args.task)
    ...
    if result.get("structural_validation"):
        ...
        if "bootstrap" in sv and "error" not in sv["bootstrap"]:
            bs = sv["bootstrap"]
            print(
                f"  Bootstrap: {bs.get('n_stable_edges_80', '?')} stable edges (B={bs.get('B', '?')})"
            )
        if "lovo" in sv and "error" not in sv["lovo"]:
            lv = sv["lovo"]
            print(
                f"  LOVO: mean F1={lv['mean_sub_f1']:.3f} "
    ...
    # ===================================================================
    ablation_result = _convert_optuna_to_ablation_format(result, args.dataset, args)
    ...
    print("  (Compatible with run_ablation_study.py pickle format)")
