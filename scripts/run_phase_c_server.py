...
FULL_PARAMS = {
    ...
    'c4':  {'n_trials': 30, 'n_vars': 10, 'n_samples': 1000, 'max_iter': 150,
...
def build_command(exp_id, script_name, params, output_path, dataset=None,
                  nsga2_results=None, seed=42):
...
def run_experiment(exp_id, script_name, description, params, output_dir,
                   dataset=None, nsga2_results=None, seed=42):
    """Run a single experiment and return result dict."""
    output_path = output_dir / f'{exp_id}_results.json'
    cmd = build_command(exp_id, script_name, params, output_path,
                        dataset=dataset, nsga2_results=nsga2_results,
                        seed=seed)
...
def main():
    parser = argparse.ArgumentParser(
        description='Phase C Server Orchestration'
    )
    parser.add_argument('--quick', action='store_true',
                        help='Quick mode: reduced parameters for local testing')
    parser.add_argument('--dataset', type=str, default=None,
                        help='Real dataset for C.1, C.2, C.5 (e.g., lucas, sachs)')
    parser.add_argument('--nsga2-results', type=str, default=None,
                        help='Cached NSGA-II results JSON for C.1')
    ...
    if args.experiments:
        ...
        experiments = [(eid, s, d, ds) for eid, s, d, ds in EXPERIMENTS
                       if eid in selected]
    else:
    ...
    print(f"PHASE C SERVER ORCHESTRATION")
    print(f"=" * 70)
    ...
    for exp_id, script_name, description, supports_dataset in experiments:
        ...
        result = run_experiment(
            exp_id, script_name, description, params, output_dir,
            dataset=dataset, nsga2_results=args.nsga2_results,
            seed=args.seed,
        )
    ...
    for r in all_results:
        ...
        if r['success']:
            ...
            n_fail += 1
        print(f"{r['exp_id']:<6} {r['description']:<35} {status:<10} "
              f"{r['time']:>7.1f}s")
