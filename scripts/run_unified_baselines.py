...
from jcce.structure_learning.markov_blanket import (
    extract_markov_blanket_indices,
...
STRUCTURE_ALGORITHMS = [
    'pc', 'ges', 'fci', 'dagma', 'notears', 'golem', 'directlingam',
]
...
def compute_mb_metrics(mb_est: list, mb_true: list) -> dict:
    ...
    return {
        'mb_f1': f1, 'mb_precision': precision, 'mb_recall': recall,
        'mb_size_est': len(est_set), 'mb_size_true': len(true_set),
        'mb_tp': tp, 'mb_fp': fp, 'mb_fn': fn,
    }
...
def run_dataset(
    ...
    if has_true_dag:
        print(f"  Ground truth available: DAG={'yes' if true_dag is not None else 'no'}, "
              f"MB={true_mb}")
    ...
    print(f"  Sample split: X_struct={X_struct.shape[0]} (70%), "
    ...
    result = {
        ...
        'n_struct': X_struct.shape[0], 'n_effect': X_effect.shape[0],
    ...
    for proc in processors:
        ...
        try:
            res = train_and_evaluate_processor(
                X_effect_std, Y_effect_int, processor_type=proc,
                n_splits=n_splits, seed=seed, n_epochs=n_epochs_cls,
            )
            res['time'] = time.time() - t0
            res['feature_set'] = 'all'
            res['n_features_used'] = d
            result['baselines'][label] = res
            print(f"    {proc:14s}: BAcc={res['balanced_acc_mean']:.3f}+-{res['balanced_acc_std']:.3f} "
    ...
    if 'error' not in shap_result:
        ...
        for proc in processors:
            ...
            try:
                res = train_and_evaluate_processor(
                    X_shap_effect, Y_effect_int, processor_type=proc,
                    n_splits=n_splits, seed=seed, n_epochs=n_epochs_cls,
                )
                res['time'] = time.time() - t0
    ...
    X_with_Y_struct = np.concatenate([X_struct_std, Y_struct_int[:, None].astype(np.float32)], axis=1)
    ...
    for algo in algorithms:
        ...
        try:
            ...
            if has_true_dag and true_dag is not None:
                ...
                print(f"    Structure: SHD={struct_metrics['shd']}, "
            ...
            print(f"    {algo.upper()}: {len(mb_indices)} MB features "
                  f"(of {d}) in {algo_time:.1f}s")

            if true_mb:
                mb_met = compute_mb_metrics(mb_indices, true_mb)
                print(f"    MB overlap: F1={mb_met['mb_f1']:.3f} "
            ...
            if len(mb_indices) == 0:
                print(f"    Empty MB — skipping classification")
            ...
            for proc in processors:
                ...
                try:
                    res = train_and_evaluate_processor(
                        X_mb_effect, Y_effect_int, processor_type=proc,
                        n_splits=n_splits, seed=seed, n_epochs=n_epochs_cls,
                    )
                    ...
                    res['mb_names'] = [feature_names[i] for i in mb_indices if i < len(feature_names)]
    ...
    try:
        ...
        for castle_task in ['regression', 'classification']:
            ...
            res = train_castle(
                X_struct_std, Y_struct_int.astype(np.float32),
                key=ck,
                task=castle_task,
        ...
        print(f"    CASTLE: BAcc={castle_entry['balanced_acc_mean']:.3f} "
        ...
        if castle_mb:
            ...
            for proc in processors:
                ...
                try:
                    res = train_and_evaluate_processor(
                        X_castle_mb_effect, Y_effect_int, processor_type=proc,
                        n_splits=n_splits, seed=seed, n_epochs=n_epochs_cls,
                    )
                    res['time'] = time.time() - t1
                    ...
                    result['baselines'][label] = res
                    print(f"    {proc:14s} (MB): BAcc={res['balanced_acc_mean']:.3f} "
                          f"F1={res['f1_mean']:.3f}")
                except Exception as e:
                    result['baselines'][label] = {'error': str(e)}
    ...
    try:
        ...
        print(f"    Running SDCD...")
        t0 = time.time()
        A_sdcd, sdcd_info = run_sdcd(X_struct_std, feature_names=feature_names, verbose=verbose >= 2)
        sdcd_time = time.time() - t0

        # Extract MB from SDCD DAG
        A_sdcd_np = np.array(A_sdcd)
        mb_sdcd = [i for i in range(d) if abs(A_sdcd_np[i, :]).sum() > 0 or abs(A_sdcd_np[:, i]).sum() > 0]
        ...
        if has_true_dag and true_dag is not None:
            ...
            print(f"    SDCD: {sdcd_entry['n_edges']} edges, SHD={s_met['shd']}, F1={s_met['edge_f1']:.3f} ({sdcd_time:.1f}s)")
        else:
        ...
        if mb_sdcd:
            ...
            for proc in processors:
                ...
                try:
                    res = train_and_evaluate_processor(
                        X_sdcd_mb, Y_effect_int, processor_type=proc,
                        n_splits=n_splits, seed=seed, n_epochs=n_epochs_cls,
                    )
                    res['time'] = time.time() - t0
    ...
    try:
        ...
        if has_true_dag and true_dag is not None:
            ...
            print(f"    DiffAN: {diffan_entry['n_edges']} edges, SHD={s_met['shd']}, F1={s_met['edge_f1']:.3f} ({diffan_time:.1f}s)")
        else:
        ...
        if mb_diffan:
            ...
            for proc in processors:
                ...
                try:
                    res = train_and_evaluate_processor(
                        X_diffan_mb, Y_effect_int, processor_type=proc,
                        n_splits=n_splits, seed=seed, n_epochs=n_epochs_cls,
                    )
                    res['time'] = time.time() - t0
    ...
    for method_name, method_key, A_baseline in methods_to_validate:
        ...
        if A_baseline.shape[0] == n_f:
            ...
            for idx in mb:
                if idx < n_f:
                    A_aug[idx, n_f] = np.max(np.abs(A_baseline[idx, :])) + 0.1  # synthetic edge weight
        else:
            A_aug = A_baseline
        ...
        try:
            ...
            if dml_result is not None:
                result['baselines'][method_key]['validation'] = {
                    ...
                    'dml_parent_results': [
                        {'feature': e.feature_name, 'ate': float(e.dml_result.ate_mean),
                         'significant': e.is_significant}
                        for e in (dml_result.parent_results or [])
                    ],
                }
                print(f"      DML: {dml_result.n_parents_significant}/{dml_result.n_parents_discovered} "
                      f"parents significant")
            else:
                print(f"      DML: no parents found")
                result['baselines'][method_key]['validation'] = {
                    'dml_n_parents': 0, 'dml_n_significant': 0,
                }
            ...
            try:
                ...
                if len(parents_of_Y) > 0:
                    ...
                    t_name = feature_names[strongest] if strongest < len(feature_names) else f'X{strongest}'
                    ...
                    result['baselines'][method_key]['validation']['cinelli_rv'] = float(rv) if rv is not None else None
                    print(f"      Cinelli RV: {rv:.3f}" if isinstance(rv, (int, float)) else f"      Cinelli: {rv}")
...
def main():
    ...
    parser.add_argument('--processors', nargs='+', default=None,
