...
def run_one(X, Y, key, name, use_structural_dml, use_pcgrad, max_iter=150):
    ...
    try:
        ...
        return {
            ...
            'ate_x0': get_ate('X0->Y'), 'ate_x4': get_ate('X4->Y'),
            'ate_x12': get_ate('X12->Y'), 'ate_x18': get_ate('X18->Y'),
        }
...
def main():
    print(f"SMOKE TEST d=20: Structural DML scaling")
    print(f"Device: {jax.devices()}")
    print("=" * 70)

    X, Y, true_effects = generate_d20_data(n=800)
    print(f"Data: n={X.shape[0]}, d={X.shape[1]}")
    print(f"True parents: X0(0.6), X4(0.4), X12(0.3), X18(0.2)\n")
    ...
    for name, sdml, pcg in configs:
        ...
        for seed in seeds:
            ...
            if 'error' in r:
                ...
                print(f"  seed={seed}: BAcc={r['bacc']:.3f} h(A)={r['h_A']:.4f} "
    ...
    print(f"{'='*80}")
    print(f"{'Config':<25s} {'BAcc':>8s} {'X0(0.6)':>8s} {'X4(0.4)':>8s} {'X12(0.3)':>9s} {'X18(0.2)':>9s} {'Time':>6s}")
    print("-" * 75)
