...
def generate_d30_data(n=1000, seed=42):
    ...
    Y = (0.5 * X[:, 0] + 0.3 * X[:, 6] + 0.4 * X[:, 12] +
         0.2 * X[:, 22] + 0.15 * X[:, 27] + rng.randn(n) * 0.3)
...
def run_one(X, Y, key, name, use_structural_dml, use_pcgrad, max_iter=100):
    ...
    try:
        A_est, _, _, metrics = learn_structure(
            data=jnp.array(X), Y=jnp.array(Y_col), Y_idx=X.shape[1],
            processor=processor, key=learn_key, processor_type='elm',
            max_iter=max_iter, lr=0.003, lambda_1=0.05, lambda_2_init=0.01,
            lambda_class=1.0, patience=25, verbose=0,
            use_amortized_effects=True,
            use_dragonnet=not use_structural_dml,
            use_structural_dml=use_structural_dml,
            use_pcgrad=use_pcgrad,
            lambda_effect=0.5, effect_warmup_iter=15,
            use_latent_confounders=False, task='classification',
        )
        ...
        return {
            'time': elapsed, 'h_A': float(metrics.get('final_h_A', 99)),
            'bacc': float(metrics.get('balanced_accuracy', 0)),
            'n_edges': int(metrics.get('n_edges', 0)),
            'ate_x0': get_ate('X0->Y'), 'ate_x6': get_ate('X6->Y'),
            'ate_x12': get_ate('X12->Y'), 'ate_x22': get_ate('X22->Y'),
...
def main():
    print(f"SMOKE TEST d=30: Structural DML at breast-cancer scale")
    print(f"Device: {jax.devices()}")
    print("=" * 80)

    X, Y, true_effects = generate_d30_data(n=1000)
    print(f"Data: n={X.shape[0]}, d={X.shape[1]}")
    print(f"True parents: X0(0.5), X6(0.3), X12(0.4), X22(0.2), X27(0.15)\n")
    ...
    for name, sdml, pcg in configs:
        ...
        for seed in seeds:
            ...
            if 'error' in r:
                ...
                print(f"  seed={seed}: BAcc={r['bacc']:.3f} h(A)={r['h_A']:.4f} edges={r['n_edges']} "
