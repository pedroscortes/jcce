...
def run_one(X, Y, key, processor_type, max_iter, use_structural_dml, use_pcgrad):
    ...
    try:
        A_est, _, _, metrics = learn_structure(
            data=jnp.array(X), Y=jnp.array(Y_col), Y_idx=Y_idx,
            processor=processor, key=learn_key,
            processor_type=processor_type,
            max_iter=max_iter, lr=0.003,
            lambda_1=0.05, lambda_2_init=0.01, lambda_class=1.0,
            ...
            lambda_effect=0.5, effect_warmup_iter=10,
        ...
        return {
            ...
            'ate_x0': get_ate('X0->Y'), 'ate_x2': get_ate('X2->Y'), 'ate_x4': get_ate('X4->Y'),
        }
...
def main():
    ...
    print(f"{'='*90}")
    print(f"{'Config':<35s} {'BAcc':>10s} {'h(A)':>10s} {'X0->Y':>10s} {'X2->Y':>10s} {'X4->Y':>10s} {'Time':>6s}")
    print("-" * 90)
    ...
    for name, results in all_results.items():
        ...
        print(f"{name:<35s} {np.mean(baccs):>5.3f}±{np.std(baccs):.3f} "
