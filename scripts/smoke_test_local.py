...
def run_config(name, X, Y, key, use_structural_dml=False, use_pcgrad=False,
               use_dragonnet=True, max_iter=80):
...
def main():
    ...
    print(f"Running on CPU — results are conclusive, just slower\n")
    ...
    r1 = run_config("DragonNet (baseline)",
                     X, Y, k1, use_structural_dml=False, use_pcgrad=False, max_iter=max_iter)

    # Config 2: Structural DML (new)
    r2 = run_config("Structural DML",
                     X, Y, k2, use_structural_dml=True, use_pcgrad=False, max_iter=max_iter)

    # Config 3: PCGrad + DragonNet
    r3 = run_config("PCGrad + DragonNet",
                     X, Y, k3, use_structural_dml=False, use_pcgrad=True, max_iter=max_iter)

    # Config 4: Structural DML + PCGrad (best of both)
    r4 = run_config("Structural DML + PCGrad",
                     X, Y, k4, use_structural_dml=True, use_pcgrad=True, max_iter=max_iter)

    # Summary
    print(f"\n\n{'='*70}")
    print("SUMMARY")
    print(f"{'='*70}")
    print(f"True effects: X0->Y=0.7, X2->Y=0.4, X4->Y=0.3\n")
    ...
    for r in [r1, r2, r3, r4]:
        if 'error' in r:
            ...
            continue
        print(f"{r['name']:<30s} {r['time']:>5.0f}s {r['h_A']:>10.6f} {r['bacc']:>6.3f} {r['n_edges']:>6d}")
