...
class TestSkipCentering:
    def test_skip_centering_improves_over_centering(self):
        ...
        processor = ELMAdapter(
            hidden_dim=32, n_hidden_nodes=64, activation="tanh", key=random.PRNGKey(0)
        )

        h_centered = _train_loop(
            X, Y, n_vars=11, processor=processor, skip_centering=False, n_steps=150
        )
        h_skip = _train_loop(X, Y, n_vars=11, processor=processor, skip_centering=True, n_steps=150)
    ...
    def test_skip_centering_breaks_below_ln2(self):
        ...
        history = _train_loop(X, Y, n_vars=11, processor=processor, skip_centering=True, n_steps=50)

        broke_through = any(h < 0.6931 for h in history)
        assert broke_through, f"Should break through ln(2) within 50 steps, min={min(history):.4f}"
    ...
    def test_skip_centering_monotonic_decrease(self):
        ...
        history = _train_loop(
            X, Y, n_vars=11, processor=processor, skip_centering=True, n_steps=100
        )
    ...
    def test_gradient_flows_with_skip_centering(self):
        ...
        processor = ELMAdapter(
            hidden_dim=16, n_hidden_nodes=32, activation="tanh", key=random.PRNGKey(0)
        )
        ...
        assert grad_norm > 0.01, f"Gradient to A should be significant, got norm={grad_norm:.6f}"
    ...
    def test_centering_difference_increases_with_imbalance(self):
        ...
        for imb in [0.5, 0.7]:
            ...
            h_cent = _train_loop(
                X, Y, n_vars=11, processor=processor, skip_centering=False, n_steps=100
            )
            h_skip = _train_loop(
                X, Y, n_vars=11, processor=processor, skip_centering=True, n_steps=100
            )
